...
export class CreateSuperAdmin extends CommandRunner {
  ...
  async run(
    ...
    await this.userService.create(createUserDto).catch(() =>
      this.logger.error(`User Already exist email: ${createUserDto.email} or mobile: ${createUserDto.mobile}`)
    );
    this.logger.log(`Creating super admin with data: ${createUserDto}`);
  }
}
