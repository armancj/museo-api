...
export class AccessConditionsService {
  ...
  async create(createDto: CreateAccessConditionDto): Promise<AccessConditionEntity> {
    const created = new this.accessConditionModel(createDto);
    const result = await created.save();
    return AccessConditionEntity.create(result.toJSON());
  }

  async findAll(): Promise<AccessConditionsEntity> {
    const results = await this.accessConditionModel.find({ deleted: false }).exec();
    return AccessConditionsEntity.create(results.map(doc => doc.toJSON()));
  }

  async findOne(uuid: string): Promise<AccessConditionEntity> {
    const result = await this.accessConditionModel.findOne({ uuid, deleted: false }).exec();
    return result ? AccessConditionEntity.create(result.toJSON()) : null;
  }

  async update(uuid: string, updateDto: CreateAccessConditionDto): Promise<AccessConditionEntity> {
