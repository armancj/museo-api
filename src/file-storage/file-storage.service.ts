...
export class FileStorageService implements FileStorageServiceModel {
  ...
  async getFileAsBase64(fileId: string): Promise<string> {
    const chunks = await this.getChunksUint8Array(fileId);

    const fileContent = Buffer.concat(chunks);
    return fileContent.toString('base64');
  }

  @OnEvent(EventEmitter.fileChunksUintArray)
  private async getChunksUint8Array(fileId: string): Promise<Uint8Array[]> {
  ...
  @OnEvent(EventEmitter.fileMetadata)
  private async getFileMetadata(id: string) {
    return await this.getFileMetadataById(id);
  }
}
