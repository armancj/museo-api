...
@Injectable()
export class ImageProcessingPipe implements PipeTransform<Express.Multer.File, Promise<FileStorageModel>> {
    constructor(@Inject(FILE_STORAGE_SERVICE_TOKEN)
                private readonly storageService: FileStorageServiceModel) {
    }

    async transform(file: Express.Multer.File, metadataArgument: ArgumentMetadata) {
        if (!file) {
        ...
        if (!allowedMimeTypes.includes(file.mimetype)) {
            throw new BadRequestException(`Invalid file type. Allowed types are: ${allowedMimeTypes.join(', ').replace(/image\//g, '')}`);
        }
