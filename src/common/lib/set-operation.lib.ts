...
export class SetOperation<TypeArray> {
  private readonly firstSet: Set<TypeArray>;
  private readonly secondSet: Set<TypeArray>;
   ...
   */
  intersection() {
    return [...this.firstSet].filter((element) => this.secondSet.has(element));
  }
   ...
   */
  exclusion() {
    const inverseOperationSet = new SetOperation({
      firstArray: [...this.secondSet],
      secondArray: [...this.firstSet],
    });
    const firstOnlyElements = this.difference();
    const secondOnlyElements = inverseOperationSet.difference();
